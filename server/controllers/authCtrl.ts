...
const authCtrl = {
  register: async (req: Request, res: Response) => {
    try {
      ...
      if (user)
        return res.status(500).json({ msg: 'Email or phone already exists' });
      ...
      if (validateEmail(account)) {
        sendMail(account, url, 'Verify your email.');
        return res.json({ msg: 'Success! Please check your email.' });
      } else if (validPhone(account)) {
        sendSms(account, url, 'Verify your number');
        return res.json({ msg: 'Success! Please check on your phone.' });
      }
    } catch (err: any) {
  ...
  activeAccount: async (req: Request, res: Response) => {
    try {
      ...
      if (user) return res.status(404).json({ msg: 'Account already exists' });
  ...
  login: async (req: Request, res: Response) => {
    try {
      ...
      const user = await Users.findOne({ account });
      if (!user) {
        return res.status(400).json({ msg: 'Account doens not exists.' });
      }
